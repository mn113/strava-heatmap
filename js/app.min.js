...
\t\t\t\tdata-athleteid="${t.id}"
            ...
            \tdata-summary="${e.map.summary_polyline}"
\t\t\t\tdata-kudos-count=${e.kudos_count}
\t\t\t\tdata-has-kudoed=${e.has_kudoed}`;return a},renderer.printRideDetails=function(e,t){var a=`<h6>
...
\t\t\t\t</div>
            \t<span class="stats">&roarr;&nbsp;${renderer.formatKm(e.distance)}</span>
            \t<span class="stats">&lrtri;&nbsp;${renderer.formatElev(e.total_elevation_gain)}</span>
\t\t\t\t<span class="stats">&raquo;&nbsp;${renderer.formatSpeed(e.average_speed)}</span>
\t\t\t\t<span class="icon kudos ${e.has_kudoed?"given":""}">${e.kudos_count}</span>
